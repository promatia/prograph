...
function graph({
    ...
    async function TypeCheckInputs(inputs, inputsSchema, parentPath, context){
        ...
        for(let name in inputs){
            ...
            if(inputSchema.type === 'scalar'){
                ...
                let promise = scalarType.incoming(input)
                    .then(async val => await inputDirectiveResolver(val, inputSchema, name, context))
                    .then(val => inputs[name] = val)
                    .catch(error => {throw new GraphError(error, path)})
    ...
    function expandField(field, path){
        ...
        if(field.type === 'paginator'){
            field.fields = {
                items: {
                    type: 'array',
                    array: {...getType(field.paginator, path), nullable: field.nullableArrayItem},
        ...
        if(field.type === 'type'){
            Object.assign(field, getType(field.value, path))
        }

        //expand array
        if(field.type === 'array'){
            field.array = getType(field.value, path)
        }
    }
    ...
    function checkForMissing(keys, type, obj){
        keys.map(item => {
            if(!obj[item]) {
                throw new GraphError(`Could not find resolver for ${type}: ${item}`, `${type}.${item}`)
            }
        })
    }
