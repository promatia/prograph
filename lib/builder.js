...
function Builder ({
    ...
    return async function graph (queryString, { context = {}, max = 1000 } = {}) {
            ...
            async function value () {
                ...
                if(schema.type === 'array') {
                    ...
                    for(let i in arrField) {
                        ...
                        promises.push(resolveInput(resolver, schema.array, `${path}.${i}`))
                    }
        ...
        async function resolveWants (fields, wants, schemas, parent) {
            ...
            for(let name in wants) {
                ...
                if ([null, undefined].includes(fields[name])) {
                    if(!schemas[name].nullable) throw new GraphError(`${path} is non-nullable, got nullable type`, path)
                    resultFields[name] = null
                    continue
                }
    ...
    function compareItems (queryTree) {
        for (let i in queryTree) {
            ...
            CheckForUnrecognisedWants(query.wants, messageSchema.fields, name)
        }
    }
