        ...
        async function resolveWant (resolverOrValue, want, schema, path) {
            ...
            async function value () {
                ...
                if(schema.type === 'type') {
                    ...
                    if(fields && fields instanceof Object === 'Object') {
                ...
                if(schema.type === 'scalar') {
                    ...
                    if(result) return await ScalarClass.outgoing(result)
                    return
                }
...
function CheckForUnrecognisedInputs (inputs, schemaInputs, parentPath) {
    for (let name in inputs) {
        ...
        if (schema.type === 'array') {
            if (input[Symbol.iterator] !== 'function') throw new GraphError(`Input ${path} is not type [${schema.value}]`, path)
            for (let i in input) {
