...
export default function Builder ({
    ...
    return async function graph (queryString, { context = {}, max = 1000 } = {}) {
        ...
        for (let i in queries) {
            let want = queries[i]
            let name = want.fieldname
            let resolver = messageResolvers[name]
            let schema = mergedMessageTypes[name]

            returnObject[want.alias] = await resolveWant(resolver, want, schema, name)
        }
        ...
        async function resolveWants (fields, wants, schemas, parent) {
            ...
            for(let i in wants) {
                ...
                promises.push(resolveWant(fields[name], want, schemas[name], path).then(val => resultFields[want.alias] = val))
            }
    ...
    function compareItems (queryTree) {
        for (let i in queryTree) {
            let alias = queryTree[i].alias
            let fieldname = queryTree[i].fieldname
            let messageSchema = mergedMessageTypes[fieldname]
            let query = queryTree[i]

            if (!messageSchema) throw new GraphError(`No recognised message type: ${fieldname} in ${alias}`, alias)
            CheckForUnrecognisedInputs(query.inputs, messageSchema.inputs, fieldname)
            CheckForMissingInputs(query.inputs, messageSchema.inputs, fieldname)

            if(!(messageSchema.fields || (messageSchema.array && messageSchema.array.fields))) continue 
            //check if there are schema wants inside array/field, throw error if missing

            let fields = messageSchema.fields || messageSchema.array.fields
            if (!query.wants) throw new GraphError(`No wants provided for ${alias}`, alias)
            CheckForUnrecognisedWants(query.wants, fields, fieldname)
        }
    }
