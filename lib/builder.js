...
function Builder ({
    ...
    return async function graph (queryString, { context = {}, max = 1000 } = {}) {
        ...
        for (let i in queryTree) {
            ...
            await TypeCheckInputs(inputs, schema.inputs, name, context)
            let resolvedMessage = await messageResolver(inputs, { wants, context })
            returnObject[name] = await resolveWantsRecursive(wants, resolvedMessage, schema, name, context, directiveResolvers)
        }

        async function callArgDirectives (args, argsSchema, path) {
            let returnArgs = {}

            for(let name in args) {
                let arg = args[name]
                let argSchema = argsSchema[name]
                let directives = argSchema.directives
                let index = 0
                
                async function value () {
                    let directiveSchema = directives[index++]
                    if(directiveSchema) {
                        let directive = directiveResolvers[directiveSchema.name]
                        
                        return await directive({name, context, value})
                    }else{
                        let value
        ...
        function resolveWantsRecursive (wants, result, schema, parent) {
            if ([null, undefined].includes(result)) {
                if(!schema.nullable) throw new GraphError(`${parent} is non-nullable, got nullable type`, parent)
                return null
            }
            ...
            for (let name in wants) {
                let wantSchema = fields[name]
                let want = wants[name]
                let path = `${parent}.${name}`
                let args = callArgDirectives(want.args, wantSchema.args, path)
                let directives = wantSchema.directives
                let index = 0
        
        
                //call directive inputs
        
                async function value () {
                    let directiveObject = directives[0]
                    if(directiveObject) {
                        let directive = directiveResolvers[directiveObject.name]
                        directive()
                    }
                }
                ...
                //call directives
                //check if result is non-nullable, throw if no value
                //check if returned type matches schema type, otherwise throw (if array, check all items)
                //wrap result in function
        
                if (typeof wants === 'object') {
                    if (result[name]) {
                        //check types
                        //check nullable
                        //call directives
