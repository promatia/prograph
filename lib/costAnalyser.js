...
export default function doCostAnalysis (queryTree, schemas, max, defaultCost) {
    ...
    function getWantsCosts (messageSchema, wants, parentCost) {
        for (let i in wants) {
            ...
            let schema = messageSchema[name]
            getSchemaCost(schema, want, parentCost)
            if (schema.type === 'type') {
                getInputCosts(schema.fields, want.args, parentCost)
                getWantsCosts(schema.fields, want.wants, parentCost)
            }
        }
    }
